...
function ageScan (str) {
    let onlyValue = [];
    let data = [];
    let thirtyTwoAge = [];
    
    str.split(', ').forEach(element => 
        {onlyValue.push(element.slice(4))
        });
    
    for (let i = 0; i < onlyValue.length; i+=2) {
        
        let obj = {}
        let key = onlyValue[i]
        let age = +(onlyValue[i+1])
        
        if (age === 32) {
            thirtyTwoAge.push(key)
        }
    ...
    let result = {};
    result.data = data;
    result.thirtyTwoAgeKeys = thirtyTwoAge;
    return result
} 
